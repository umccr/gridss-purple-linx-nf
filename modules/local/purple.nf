...
process PURPLE {
  ...
  input:
  tuple val(meta), path(amber), path(cobalt), path(sv_soft_vcf), path(sv_soft_vcf_index), path(sv_hard_vcf), path(sv_hard_vcf_index), path(smlv_tumour_vcf)
  ...
  script:
  """
  # If we have the tumour small variant VCF then filter records missing AD and set argument value
  if [[ "${smlv_tumour_vcf.name}" != 'NOFILE' ]]; then
    bcftools filter -Oz -e 'FORMAT/AD[*]="."' "${smlv_tumour_vcf}" > tumour_small_variants_filtered.vcf.gz
    smlv_tumour_vcf_arg='-somatic_vcf tumour_small_variants_filtered.vcf.gz'
  else
    smlv_tumour_vcf_arg=''
  fi
  # Run PURPLE
